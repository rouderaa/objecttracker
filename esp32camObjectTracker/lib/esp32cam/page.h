// Uncompressed version of the html of the esp32 camera configuration page
// Also reduced number of fields slightly in regard to the original
const char *page = R"(
<!doctype html>
<html>
    <head>
            ...
            <div id="content">
                <div id="sidebar">
                    ...
                    <nav id="menu">
                        ...
                        <div class="input-group" id="level-group">
                          <label for="level_intensity">Red level</label>
                          <div class="range-min">0</div>
                          <input type="range" id="red_level" min="0" max="255" value="0" class="default-action">
                          <div class="range-max">255</div>
                        </div>
                        <div class="input-group" id="level-group">
                          <label for="level_intensity">Green level</label>
                          <div class="range-min">0</div>
                          <input type="range" id="green_level" min="0" max="255" value="0" class="default-action">
                          <div class="range-max">255</div>
                        </div>
                        <div class="input-group" id="level-group">
                          <label for="level_intensity">Blue level</label>
                          <div class="range-min">0</div>
                          <input type="range" id="blue_level" min="0" max="255" value="0" class="default-action">
                          <div class="range-max">255</div>
                        </div>
                        <div class="input-group" id="autorefresh-group">
                            <label for="refreshsw">Auto refresh</label>
                            <div class="switch">
                                <input id="refreshsw" type="checkbox" class="default-action">
                                <label class="slider" for="refreshsw"></label>
                            </div>
                        </div>
                        <section id="buttons">
                            <button id="get-still">Photo</button>
                            <button id="calibrate">Calibrate</button>
                            <button id="toggle-stream">Movie</button>
                        </section>
                ...
                <figure>
                    <div id="stream-container" class="image-container hidden" >
                        ...
                        <!-- <img id="filtered-stream" src="" crossorigin> -->
                    </div>
                </figure>
            </div>
        </section>
        <script>
// Global variable to store the interval ID so we can clear it later
let refreshIntervalId = null;

// Function to refresh webcam image
function refreshWebcamImage() {
  const img = document.getElementById('webcamImage');
  const webcamUrl = '${baseHost}/bmp?_cb=${Date.now()}';
  ...
  refreshIntervalId = setInterval(() => {
    img.src = webcamUrl + '?t=' + new Date().getTime();
  }, 1000);
}

// Function to stop refreshing the image
...
function handleCheckboxChange() {
  const refreshToggle = document.getElementById('refreshsw');
  
  if (refreshToggle.checked) {
    refreshWebcamImage();
    console.log('Image refresh activated');
  } else {
    stopRefreshingImage();
    console.log('Image refresh deactivated');
  }
}

// Initialize everything when the DOM is fully loaded
document.addEventListener('DOMContentLoaded', function() {
  // Set up the event listener for checkbox
  const refreshToggle = document.getElementById('refreshToggle');
  refreshToggle.addEventListener('change', handleCheckboxChange);
...
*/
  ...
  */
  ...
  const stopStream = () => {
    window.stop();
    streamButton.innerHTML = 'Start Movie'
  }

  const startStream = () => {
    view.src = `${streamUrl}/stream`
    show(viewContainer)
    streamButton.innerHTML = 'Stop Movie'
  }

  // Attach actions to buttons
  stillButton.onclick = () => {
    console.log('still button')
    stopStream()
    view.src = `${baseHost}/bmp?_cb=${Date.now()}`
    show(viewContainer)
  }

  calibrateButton.onclick = () => {
    console.log('calibrate button')

    const query = `${baseHost}/calibrate`
  ...
  streamButton.onclick = () => {
    const streamEnabled = streamButton.innerHTML === 'Stop Movie'
    if (streamEnabled) {
      stopStream()
    } else {
      startStream()
